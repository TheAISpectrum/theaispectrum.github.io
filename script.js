...
const founderSlug = f => f.slug || f.name.toLowerCase().normalize('NFKD').replace(/[\u0300-\u036f]/g, '').replace(/[^a-z0-9]+/g, '-').replace(/^-|-$/g, '');
...
function chrome(active = page) {
  document.querySelector('#header').innerHTML = `<div class="header-inner"><a class="brand" href="/" aria-label="The AI Spectrum home"><img src="assets/favicon.svg" width="33" height="33" alt=""><span>The AI Spectrum<span class="brand-period">.</span></span></a><nav aria-label="Main navigation"><a href="/" ${active === 'home' ? 'aria-current="page"' : ''}>Home</a>${Object.entries(areas).map(([key,a]) => `<a href="${topicLink(key)}" ${active === key ? 'aria-current="page"' : ''}>${a.label}</a>`).join('')}<a href="/about" ${active === 'about' || active === 'founder' ? 'aria-current="page"' : ''}>About us</a></nav></div>`;
  ...
  template.content.querySelectorAll('*').forEach(el=>{
    for (const attr of [...el.attributes]) {
      ...
      if (['href','src','xlink:href','action'].includes(attr.name)) {
        try { if (!['https:','http:','mailto:'].includes(new URL(attr.value,document.baseURI).protocol)) el.removeAttribute(attr.name); }
...
async function renderRoute({initial=false,restoreScroll}={}) {
  ...
  try {
    ...
    document.querySelector('meta[name="description"]').content = 'Four perspectives on artificial intelligence: its footprint, physical infrastructure, usefulness, and influence on how we think.';
    chrome(page);
    ...
    console.error(error);
    missing('We lost the signal', 'This story could not be loaded. Try again in a moment, or explore another perspective.', '/', 'Back to home', 'offline');
  } finally {
...
async function start() {
  ...
  try {
    ...
    console.error(error);
    missing('We lost the signal', 'Please reload to reconnect with the spectrum.', '/', 'Try the home page', 'offline');
  }
}
start();
