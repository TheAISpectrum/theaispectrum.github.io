...
const areas = {
  ...
  'physical-ai': { number: '02', label: 'Physical AI', full: 'Physical implementation of AI', color: '#dfa57c', question: 'What is AI actually built on?', description: 'Chips, memory, servers, and data centres. We explore the physical infrastructure that makes AI possible, from the processor to the systems that keep it running.' },
...
const illustrations = {
  environment: 'Contour lines surrounding a computing core',
  'physical-ai': 'An AI chip connected to rows of data centre servers',
  productivity: 'Lines becoming an organized stack of documents',
  cognition: 'Connections fading across an abstract network of thought'
};
// Font Awesome Free 6.7.2, bundled locally; see vendor/fontawesome/LICENSE.txt.
const icon = (direction = 'right') => `<svg class="icon icon-${direction}" aria-hidden="true" focusable="false"><use href="vendor/fontawesome/arrows.svg#arrow-${['up-right','down-right'].includes(direction) ? 'right' : direction}"></use></svg>`;
...
function home(posts) {
  main.innerHTML = `<section class="hero"><div class="hero-copy"><p class="eyebrow"><span class="status-dot"></span> AN INDEPENDENT LOOK AT ARTIFICIAL INTELLIGENCE</p><h1>There’s more<br>to AI than<br><em>the algorithm.</em></h1><p class="hero-intro">A tool. A footprint. A physical foundation.<br>A shift in how we think.</p><p class="hero-description">We explore the full spectrum of artificial intelligence — what it makes possible, and what it asks of us.</p><a class="button" href="#perspectives">Explore the spectrum <span aria-hidden="true">${icon('down-right')}</span></a></div><figure class="hero-art"><img src="assets/spectrum.svg" width="760" height="690" alt="A single beam of light enters a prism and separates into four colored paths: environment, physical AI, productivity, and human cognition."><figcaption><span>ONE TECHNOLOGY.</span><span>FOUR WAYS OF SEEING IT.</span></figcaption></figure></section><section class="perspectives" id="perspectives"><div class="section-heading"><p class="eyebrow">THE FOUR PERSPECTIVES</p><p>Follow a question. Find a different angle.</p></div><div class="perspective-grid">${Object.entries(areas).map(([key,a]) => `<a class="perspective" href="${topicLink(key)}" style="--accent:${a.color}"><div class="perspective-top"><span>${a.number} /</span><span aria-hidden="true">${icon('up-right')}</span></div><h2>${a.label}</h2><p>${a.question}</p><span class="perspective-bottom">Explore the theme <span aria-hidden="true">${icon('right')}</span></span></a>`).join('')}</div></section><section class="latest"><div class="section-heading"><div><p class="eyebrow">NOTES FROM ACROSS THE SPECTRUM</p><h2>The latest thinking<span class="brand-period">.</span></h2></div><span class="small-note">Four themes. An open mind.</span></div><div class="posts-grid">${posts.map(p => card(p)).join('')}</div></section><section class="motive"><p class="eyebrow">WHY WE’RE HERE</p><h2>Curiosity over hype.<br><em>Understanding over assumptions.</em></h2><div><p>AI is becoming part of everyday life. We believe understanding it means looking at the whole picture: the resources it consumes, the systems it powers, the work it changes, and the habits it shapes.</p><p>This is our space to ask better questions, connect different perspectives, and make a complex subject a little more human.</p><a class="text-link" href="about.html">The people behind the questions <span aria-hidden="true">${icon('up-right')}</span></a></div></section>`;
}
function theme(key, posts) {
  const a = areas[key];
  document.title = `${a.label} — The AI Spectrum`;
  main.innerHTML = `<section class="theme-hero" style="--accent:${a.color}"><div><a class="breadcrumb" href="index.html#perspectives">${icon('left')} All perspectives</a><p class="eyebrow">PERSPECTIVE ${a.number} / ${a.full.toUpperCase()}</p><h1>${a.label}<span>.</span></h1><h2>${a.question}</h2><p>${a.description}</p></div><img src="assets/${key}.svg" width="720" height="460" alt="${esc(illustrations[key])}"></section><section class="latest theme-latest"><div class="section-heading"><h2>Latest stories</h2><span class="small-note">${posts.filter(p=>p.area===key).length} article${posts.filter(p=>p.area===key).length===1?'':'s'} · Newest first</span></div><div class="posts-grid topic-posts">${posts.filter(p=>p.area===key).map(p=>card(p)).join('') || '<p>New perspectives are on their way. Check back soon.</p>'}</div></section>`;
}
function about(founders) {
  document.title = 'About us — The AI Spectrum';
  main.innerHTML = `<section class="about-intro"><p class="eyebrow">THE PEOPLE BEHIND THE PERSPECTIVES</p><h1>Four minds.<br><em>A wider view.</em></h1><div><p>We’re a team of four, connected by a shared curiosity about artificial intelligence and the world it’s changing.</p><p>Each of us follows a different thread. Together, we want to look beyond easy answers — and create a space where useful ideas and difficult questions can sit side by side.</p></div></section><section class="team-section"><div class="section-heading"><h2>Meet the team<span class="brand-period">.</span></h2><p>Select a portrait to read their story ${icon('up-right')}</p></div><div class="team-grid">${founders.map(f=>`<a class="person-card" href="founder.html?id=${encodeURIComponent(f.id)}" style="--accent:${areas[f.area].color}"><div class="portrait-wrap"><img src="${esc(f.image)}" alt="${esc(f.imageAlt)}" width="600" height="700"><span class="portrait-link">Read my story ${icon('up-right')}</span></div><div class="person-meta">${tag(f.area)}<h3>${esc(f.name)}</h3><p>${esc(f.line)}</p></div></a>`).join('')}</div></section><section class="editorial"><p class="eyebrow">OUR EDITORIAL APPROACH</p><h2>Look closely.<br>Ask honestly.</h2><div><p>We aim to explain clearly, link to our sources, and distinguish evidence from opinion. A promising application can have real costs; a serious concern can still deserve careful scrutiny.</p><p>We’re here to keep learning, and to bring you into that process.</p></div></section>`;
}
...
function missing(title='Page not found', message='There’s more to explore. Let’s take you back to the spectrum.', href='index.html', label='Back to home') {
  ...
  main.innerHTML = `<section class="error-page"><p class="eyebrow">A DIFFERENT DIRECTION</p><h1>${esc(title)}.</h1><p>${esc(message)}</p><a class="button" href="${href}">${label} ${icon('right')}</a></section>`;
}
